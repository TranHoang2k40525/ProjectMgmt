...
--
--  Phiên bản: 3.0  (chuyển từ SQL Server sang MySQL, bổ sung module AI Assignment & Dataset)
--
...
CREATE TABLE `Issue` (
    ...
    CONSTRAINT `CK_Issue_NoSelfParent` CHECK (`ParentId` IS NULL OR `ParentId` <> `Id`),
...
CREATE TABLE `IssueAssignmentHistory` (
    ...
    KEY `IX_IssueAssignHistory_Source` (`AssignmentSource`),
...
CREATE TABLE `AcceptanceCriteria` (
    ...
    KEY `IX_AcceptanceCriteria_Source` (`Source`),
...
CREATE TABLE `AiSuggestedTask` (
    ...
    KEY `IX_AiSuggestedTask_Action` (`UserAction`) COMMENT 'Truy vấn export dataset',
...
CREATE TABLE `AiAssignmentDecision` (
    ...
    KEY `IX_AiAssignmentDecision_Outcome` (`Outcome`),
