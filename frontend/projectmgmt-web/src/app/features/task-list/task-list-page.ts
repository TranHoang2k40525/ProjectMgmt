  ...
  template: `
    <div class="flex flex-col gap-6 animate-fade-in font-body text-slate-900 dark:text-slate-100">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <!-- Left Filter Panel -->
        <div class="p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 shadow-xs flex flex-col gap-4 select-none">
        ...
        <div class="lg:col-span-3 p-5 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 shadow-xs flex flex-col gap-4">
            ...
            <table class="w-full text-left text-sm border-collapse">
              ...
              <tbody class="divide-y divide-slate-100 dark:divide-slate-800/60">
                @for (item of filteredItems(); track item.id; let i = $index) {
                  ...
                  >
                    <td class="py-3.5 px-4 font-mono text-slate-400 text-sm">{{ i + 1 }}</td>
                    <td class="py-3.5 px-4 font-mono font-bold text-primary text-sm">{{ item.issueKey }}</td>
                    <td class="py-3.5 px-4 font-semibold text-slate-900 dark:text-white group-hover:text-primary transition-colors text-sm">
                      {{ item.title }}
                    </td>
                    ...
                    <td class="py-3.5 px-4 text-slate-500 font-medium text-sm">
                      {{ item.sprintName }}
                    </td>
                  </tr>
                }
